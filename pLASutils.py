...
class p_lasReader: 
	def __init__(self,filename=''):
		self.re_parm  = re.compile('([A-Z_][A-Z_ ]+\.[\w%/]*) (.*):(.*)')
	...
	def readFile(self,filename):
		...
		self.curveNames = []
		self.curves = self.parseSection('~CURVE',self.curveNames,True)
	...
	def createDataArrays(self):
		...
		if not self.records.has_key('~ASCII'): 
			self.records['~ASCII'] = self.records['~A']
	...
	def parseSection(self,sectionName,order=None,condition=False):
		obj = {}
		...
		for ln in self.records[sectionName]:
			...
			if ma:
				...
				if condition: cname = cname.strip() 
	...
	def getCurveData(self,csName=None):
